...
const OrderTable = ({ orders }) => {
  const OrderRow = ({ order }) => (
    <tr className="border-b border-slate-200 hover:bg-slate-50">
      <td className="p-4 text-slate-700 font-medium">{order.customerName}</td>
      ...
      <td className="p-4 text-slate-600">${order.total.toFixed(2)}</td>
    ...
    <div className="mt-6 bg-white rounded-lg shadow-sm border border-slate-200 overflow-x-auto">
      <table className="min-w-full">
        <thead className="border-b border-slate-200 bg-slate-50">
          <tr>
            <th className="p-4 text-left text-sm font-semibold text-slate-500 uppercase">Customer</th>
            ...
            <th className="p-4 text-left text-sm font-semibold text-slate-500 uppercase">Date</th>
